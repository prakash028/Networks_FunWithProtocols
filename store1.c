...
# define udp_store2port1 "9532"
# define udp_store2port2 "11532"
# define udp_port1 "5532"
# define udp_port2 "6532"
# define udp_port3 "7532"
# define udp_port4 "8532"
...
int main(){
    char *selectedport,*portforstore2,*recfrom,*port;
    ...
    /*
     * Reading the file Store-1.txt by calling the function file_reader
     * The  vector read,is stored in global outlet vector
     */
    
    if(!file_reader("Store-1.txt")){
        return 0;
    ...
    printf("\n\nPhase 1: store_1 has TCP port number %d and IP address %s\n",ntohs(s.sin_port),inet_ntoa(*addr_list[0]));
    ...
    if((bytes_sent = send(sockfd, outletVector, sizeof outletVector, 0)) == -1) {
       ...
       printf(">");
       printf( "for Store1 has been sent to the central warehouse\n");
    }
    
    printf("\nEnd of Phase 1 for Store 1\n\n\n");
     ...
     * !st round : Store1 will recieve the truck vector from warehouse and send updated truck vector to store2
     * 2nd round: Store1 will recieve the truck vector from store4 and send the updated truck vector to store2
     */
    ...
    for(j=0;j<2;j++){
        ...
        */
        ...
        printf("Phase 2: store_1 has UDP port number %s  and IP address %s",selectedport,ipstr);
        ...
        printf("\nPhase 2: Store_1 received the truck-vector <");
        for(i=0;i<3;i++){
        ...
        if(j==0){
            //round 1
            portforstore2=udp_store2port1;
            port = udp_port2;
        }else{
            //round 2
            portforstore2=udp_store2port2;
            port = udp_port4;
        }

        /*
         * START OF PHASE_2 PART 2
         * Sending the updated truck vector to the store2
        ...
        if ((status = getaddrinfo(host,portforstore2, &hints, &serverinfo)) != 0) {
            fprintf(stderr, "getaddrinfo: %s\n", gai_strerror(status));
            return 1;
        }
        
        // binding datagram socket to the static udp port of the sender(store1) as given in the project specification.
        ...
        printf("\nPhase 2: Store_1 has UDP port  %s  and IP address %s ",port,ipstr);
        ...
        printf(">has been sent to store-2\n");
...
int file_reader(char *filename){
    ...
    char lines[100];
    char *tk1,*tk2;
    //getting the file Store-1.txt and extracting the vector
    if((file = fopen(filename,"r")) == NULL){
        perror("Please change the outlet vector filename to Store-1.txt");
        return 0;
    }
    //reading lines  
    i=0;
