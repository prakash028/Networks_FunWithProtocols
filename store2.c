...
# define udp_store3port1 "13532"
# define udp_store3port2 "15532"
# define udp_port1 "9532"
# define udp_port2 "10532"
# define udp_port3 "11532"
# define udp_port4 "12532"
...
int main(){
    char *selectedport,*portforstore3,*recfrom,*port;
    ...
    /*
     * Reading the file Store-2.txt by calling the function file_reader
     * The  vector read,is stored in global outlet vector
     */
    
    if(!file_reader("Store-2.txt")){
        return 0;
    ...
    printf("\n\nPhase 1: store_2 has TCP port number %d and IP address %s\n",ntohs(s.sin_port),inet_ntoa(*addr_list[0]));
    ...
    if((bytes_sent = send(sockfd, outletVector, sizeof outletVector, 0)) == -1) {
       ...
       printf(">");
       printf( "for Store2 has been sent to the central warehouse\n");
    }
    
    printf("\nEnd of Phase 1 for Store 2\n\n\n");
     ...
     * !st round : Store2 will recieve the truck vector from store1 and send updated truck vector to store3
     * 2nd round: Store2 will recieve the truck vector from store1 and send the updated truck vector to store3
     */
    ...
    for(j=0;j<2;j++){
        ...
        /*
         * getting the address information of host at selectedport
         * if successful,the results are used to obtain a socket descriptor
         */
        ...
        */
        ...
        printf("Phase 2: store_2 has UDP port number %s  and IP address %s",selectedport,ipstr);
        ...
        printf("\nPhase 2: Store_2 received the truck-vector <");
        for(i=0;i<3;i++){
        ...
        if(j==0){
            //round 1
            portforstore3=udp_store3port1;
            port = udp_port2;
        }else{
            //round 2
            portforstore3=udp_store3port2;
            port = udp_port4;
        }

        /*
         * START OF PHASE_2 PART 2
         * Sending the updated truck vector to the store3
        ...
        if ((status = getaddrinfo(host,portforstore3, &hints, &serverinfo)) != 0) {
            fprintf(stderr, "getaddrinfo: %s\n", gai_strerror(status));
            return 1;
        }
        
        // binding datagram socket to the static udp port of the sender(store2) as given in the project specification.
        ...
        printf("\nPhase 2: Store_2 has UDP port  %s  and IP address %s ",port,ipstr);
        ...
        printf(">has been sent to store-3\n");
        printf("Phase2: Store-2 updated outlet-vector is <");
...
int file_reader(char *filename){
    ...
    char lines[100];
    char *tk1,*tk2;
    //getting the file Store-2.txt and extracting the vector
    if((file = fopen(filename,"r")) == NULL){
        perror("Please change the outlet vector filename to Store-2.txt");
        return 0;
    }
    //reading lines  
    i=0;
